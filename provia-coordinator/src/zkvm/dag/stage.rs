...
pub use crate::subprotocols::sumcheck::{BatchedSumcheckInstance, PublicSumcheckInstance, Rep3SumcheckInstance};
...
>
{
    fn stage1_prove(&mut self, _sm: &mut StateManager<'_, F, ProofTranscript, PCS>) -> Result<(), eyre::Report> {
        Ok(())
    }
...
impl<F, ProofTranscript, PCS, N> SumcheckStagesCoordinator<F, ProofTranscript, PCS, N> for Rep3JoltDagStages
where
    F: JoltField,
    ProofTranscript: Transcript,
    ...
    N: Rep3NetworkCoordinator,
{
    ...
    fn stage2_instances(
        ...
        let spartan_instances: Vec<BatchedSumcheckInstance<F, ProofTranscript>> =
            spartan_instances.into_iter().map(BatchedSumcheckInstance::Secret).collect();
        ...
        network.broadcast_request((ram_rwc.gamma(), ram_rwc.input_claim(), output.r_address().to_vec()))?;
        ...
        let mut stage2_instances: Vec<BatchedSumcheckInstance<F, ProofTranscript>> = Vec::with_capacity(
            spartan_instances.len() + registers_instances.len() + ram_instances.len() + lookups_instances.len(),
        );
    ...
    fn stage3_instances(
        ...
        let registers_val_claim = state
            ...
            .1;
        let product_input_claim =
            state.accumulator.get_virtual_polynomial_opening(VirtualPolynomial::Product, SumcheckId::SpartanOuter).1;
        ...
        let (r_cycle_point, next_pc_eval) =
            state.accumulator.get_virtual_polynomial_opening(VirtualPolynomial::NextPC, SumcheckId::SpartanOuter);
        let (_, next_unexpanded_pc_eval) = state
            .accumulator
            .get_virtual_polynomial_opening(VirtualPolynomial::NextUnexpandedPC, SumcheckId::SpartanOuter);
        let (_, next_is_noop_eval) =
            state.accumulator.get_virtual_polynomial_opening(VirtualPolynomial::NextIsNoop, SumcheckId::SpartanOuter);
        ...
        let (_, rv_claim) =
            state.accumulator.get_virtual_polynomial_opening(VirtualPolynomial::LookupOutput, SumcheckId::SpartanOuter);
        let (_, left_operand_claim) = state
            .accumulator
            .get_virtual_polynomial_opening(VirtualPolynomial::LeftLookupOperand, SumcheckId::SpartanOuter);
        let (_, right_operand_claim) = state
            .accumulator
    ...
    fn stage4_instances(
        ...
        let (ram_instances, ram_init) = Rep3RamDag::stage4_instances_with_init::<F, ProofTranscript, PCS>(state);
        ...
        eyre::ensure!(
            state.accumulator.openings.contains_key(&jolt_core::poly::opening_proof::OpeningId::Virtual(
                VirtualPolynomial::InstructionRa,
                SumcheckId::InstructionReadRaf
            )),
            "missing InstructionRa opening (expected from stage3 ReadRaf)"
        );
        let (ra_point, ra_claim) = state
            .accumulator
        ...
        network.broadcast_request((ram_init, bytecode_init, ra_claim, r_address.to_vec(), r_cycle.to_vec()))?;
