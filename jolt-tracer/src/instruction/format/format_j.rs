...
impl InstructionRegisterState for RegisterStateFormatJ {
    ...
    fn random(rng: &mut rand::rngs::StdRng) -> Self {
        use rand::RngCore;
        Self { rd: (rng.next_u64(), rng.next_u64()) }
    }
...
impl InstructionFormat for FormatJ {
    ...
    fn random(rng: &mut rand::rngs::StdRng) -> Self {
        ...
        use rand::RngCore;
        Self { rd: (rng.next_u64() as u8 % RISCV_REGISTER_COUNT), imm: rng.next_u64() }
    }
}

impl From<NormalizedOperands> for FormatJ {
    fn from(operands: NormalizedOperands) -> Self {
        Self { rd: operands.rd, imm: operands.imm as u64 }
    }
}

impl From<FormatJ> for NormalizedOperands {
    fn from(format: FormatJ) -> Self {
        Self { rs1: 0, rs2: 0, rd: format.rd, imm: format.imm as i128 }
    }
}
