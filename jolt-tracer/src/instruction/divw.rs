...
use super::{
    format::format_r::FormatR, mul::MUL, virtual_advice::VirtualAdvice, virtual_assert_eq::VirtualAssertEQ,
...
impl RISCVTrace for DIVW {
    fn trace(&self, cpu: &mut Cpu, trace: Option<&mut Vec<Cycle>>) {
        ...
        if let Instruction::VirtualAdvice(instr) = &mut inline_sequence[0] {
            instr.advice = Some(quotient as XlenInt);
        } else {
            panic!("Expected Advice instruction");
        }
        if let Instruction::VirtualAdvice(instr) = &mut inline_sequence[1] {
            instr.advice = Some(remainder as XlenInt);
        } else {
            panic!("Expected Advice instruction");
        }
