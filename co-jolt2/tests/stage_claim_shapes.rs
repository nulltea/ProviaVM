...
use jolt_core::zkvm::{JoltProverPreprocessing, JoltSharedPreprocessing, JoltVerifierPreprocessing};
...
fn make_state_manager(trace_length: usize, ram_k: usize) -> StateManagerCoordinator<'static, F, ProofTranscript, PCS> {
    ...
    let verifier_preprocessing: &'static JoltVerifierPreprocessing<F, PCS> =
...
fn seed_spartan_outer_openings(sm: &mut StateManagerCoordinator<'_, F, ProofTranscript, PCS>, log_t: usize) {
...
fn stage_cache_openings_shapes_do_not_panic() {
    ...
    let reg_opening_point =
        OpeningPoint::<BIG_ENDIAN, F>::new(sm.transcript.challenge_vector_optimized::<F>(5 + log_t));
    ...
    let raf_opening_point =
        OpeningPoint::<BIG_ENDIAN, F>::new(sm.transcript.challenge_vector_optimized::<F>(ram_k.log_2()));
    ...
    let ram_opening_point =
        OpeningPoint::<BIG_ENDIAN, F>::new(sm.transcript.challenge_vector_optimized::<F>(ram_k.log_2() + log_t));
...
fn output_sumcheck_expected_output_claim_matches_formula() {
    ...
    let got = <Rep3OutputSumcheck<F> as Rep3SumcheckInstance<F, ProofTranscript>>::expected_output_claim(
        &output,
        &sm.accumulator,
        &opening_point.r,
    );
    ...
    let io_mask = RangeMaskPolynomial::<F>::new(
        ...
        .unwrap() as u128,
        remap_address(RAM_START_ADDRESS, &sm.program_io.memory_layout)
            .unwrap() as u128,
    );
