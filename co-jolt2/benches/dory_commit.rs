...
use mpc_core::protocols::rep3::test_utils::{
    run_rep3_local_test_with_coordinator, LocalRep3TestWorkerNet,
};
...
fn main() {
    ...
    println!(
        "commit_rep3 (field shares): {:?} / iter  ({ITERS} iters, total {:?})",
        per_iter_f, durations_f[0]
    );
    ...
    println!(
        "  Ring shares  preprocessing:               {:?} / commit",
        preproc_per_commit
    );
