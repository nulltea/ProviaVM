...
impl<F: JoltField> PublicSumcheckInstanceWorker<F> for ReadRafSumcheck<F> {
    ...
    fn compute_prover_message_public(
        ...
        // Build full evals at {0, 1, ..., degree}.
        // base = [y0, y2] (degree = 2 in log_K phase) or [y0, y2, ..., y_{d+1}] (cycle phase).
        let y0 = base[0];
    ...
    fn cache_openings_public(
        ...
        for i in 0..d {
            ...
            let claim = self.ra_final_claim(i);
            claims.push(claim);
            ...
            accumulator.append_sparse(
                ...
                r_addr_chunk,
                &r_cycle.r,
                vec![rep3_arith::promote_to_trivial_share(PartyID::ID0, claim)],
            );
        }
