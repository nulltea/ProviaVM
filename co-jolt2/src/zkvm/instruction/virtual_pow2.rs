...
impl<const XLEN: usize> Rep3LookupQuery<XLEN> for Rep3RISCVCycle<VirtualPow2> {
    ...
    fn to_lookup_output_batched<'a, F: JoltField, N: Rep3Network>(
        ...
        itertools::izip!(steps, out).for_each(|(step, out)| {
            ...
            *out = FutureRep3Ring::Ready(
                rep3::arithmetic::promote_to_trivial_share(io_ctx.id, F::from(1u64 << (val % XLEN as u64))).into(),
            );
        });
        Ok(())
    }
...
impl<const XLEN: usize> Rep3LookupQuery<XLEN> for Rep3RISCVCycle<VirtualPow2I> {
    fn to_instruction_inputs(&self) -> (Rep3Operand, Rep3Operand) {
