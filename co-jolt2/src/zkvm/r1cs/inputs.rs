...
impl<F: JoltField> Rep3R1CSCycleInputs<F> {
    pub fn from_witness(party_id: mpc_core::protocols::rep3::PartyID, row: Rep3CycleWitnessRef<'_, F>) -> Self {
        let fb = row.flags_bits();
        let mask_left_rs1 = 1u32 << (CircuitFlags::LeftOperandIsRs1Value as usize);
        let mask_left_pc = 1u32 << (CircuitFlags::LeftOperandIsPC as usize);
        let mask_right_rs2 = 1u32 << (CircuitFlags::RightOperandIsRs2Value as usize);
        let mask_right_imm = 1u32 << (CircuitFlags::RightOperandIsImm as usize);

        let left_input = if (fb & mask_left_rs1) != 0 {
            row.rs1_value()
        } else if (fb & mask_left_pc) != 0 {
            arithmetic::promote_to_trivial_share(party_id, F::from_u64(row.unexpanded_pc()))
        } else {
            Rep3PrimeFieldShare::zero_share()
        };

        let right_input = if (fb & mask_right_rs2) != 0 {
            row.rs2_value()
        } else if (fb & mask_right_imm) != 0 {
            arithmetic::promote_to_trivial_share(party_id, F::from_i128(row.imm()))
        } else {
            Rep3PrimeFieldShare::zero_share()
        };

        let lookup_output = row.lookup_output();
    ...
    N: Rep3NetworkWorker,
{
    let cycle_witness = state
        ...
        .cycle_witness
        .as_ref()
        .expect("cycle_witness missing; generate witness first");
    ...
    let lookup_outputs = &cycle_witness.lookup_output;

    let party_id = state.io_ctx.party_id();

    eyre::ensure!(
        lookup_outputs.len() == trace_len,
        "lookup_outputs length mismatch: got {}, expected {}",
        lookup_outputs.len(),
        trace_len
    );
    ...
    let advice = &cycle_witness.advice;

    let rs1_field = &cycle_witness.rs1_value;
    let rs2_field = &cycle_witness.rs2_value;
    let rd_write_field = &cycle_witness.rd_write_value;
    let ram_read_field = &cycle_witness.ram_read_value;
    let ram_write_field = &cycle_witness.ram_write_value;

    // Prepare batched shared×shared products (rows where both instruction inputs are shared).
    let mask_left_rs1 = 1u32 << (CircuitFlags::LeftOperandIsRs1Value as usize);
    let mask_left_pc = 1u32 << (CircuitFlags::LeftOperandIsPC as usize);
    let mask_right_rs2 = 1u32 << (CircuitFlags::RightOperandIsRs2Value as usize);
    let mask_right_imm = 1u32 << (CircuitFlags::RightOperandIsImm as usize);
    let mask_add = 1u32 << (CircuitFlags::AddOperands as usize);
    let mask_sub = 1u32 << (CircuitFlags::SubtractOperands as usize);
    let mask_mul = 1u32 << (CircuitFlags::MultiplyOperands as usize);
    let mask_advice = 1u32 << (CircuitFlags::Advice as usize);
    ...
    let mut acc_public: Vec<F> = vec![F::zero(); n_inputs];
    let mut acc_shared: Vec<Rep3PrimeFieldShare<F>> =
    ...
    for x1 in 0..eq_one.len() {
        let eq1_val = eq_one[x1];
        let mut inner_public: Vec<F> = vec![F::zero(); n_inputs];
        let mut inner_shared: Vec<Rep3PrimeFieldShare<F>> =
            vec![Rep3PrimeFieldShare::zero_share(); n_inputs];
        ...
        for x2 in 0..eq_two.len() {
            ...
            let next_pc = if t + 1 < trace_len { pc[t + 1] } else { 0 };
            let next_is_noop = if t + 1 < trace_len {
                (flags_bits[t + 1] & (1u32 << (CircuitFlags::IsNoop as usize))) != 0
            } else {
                true
            };
            inner_public[idx_next_unexp] += F::from_u64(next_unexp) * eq2_val;
            inner_public[idx_next_pc] += F::from_u64(next_pc) * eq2_val;
            inner_public[idx_next_is_noop] += F::from_bool(next_is_noop) * eq2_val;

            inner_shared[idx_lookup_output] += arithmetic::mul_public(lookup_outputs[t], eq2_val);

            inner_shared[idx_rs1] += arithmetic::mul_public(rs1_field[t], eq2_val);
            inner_shared[idx_rs2] += arithmetic::mul_public(rs2_field[t], eq2_val);
            inner_shared[idx_rd_write] += arithmetic::mul_public(rd_write_field[t], eq2_val);
            inner_shared[idx_ram_read] += arithmetic::mul_public(ram_read_field[t], eq2_val);
            inner_shared[idx_ram_write] += arithmetic::mul_public(ram_write_field[t], eq2_val);

            // Per-row instruction inputs (for committed inputs, Product + lookup operands).
            let fb = flags_bits[t];
            let left_is_shared = (fb & mask_left_rs1) != 0;
            let left_is_pc = (fb & mask_left_pc) != 0;
            let right_is_shared = (fb & mask_right_rs2) != 0;
            let right_is_imm = (fb & mask_right_imm) != 0;

            // LeftInstructionInput (committed)
            if left_is_shared {
                inner_shared[idx_left] += arithmetic::mul_public(rs1_field[t], eq2_val);
            } else if left_is_pc {
                inner_public[idx_left] += F::from_u64(unexpanded_pc[t]) * eq2_val;
            }

            // RightInstructionInput (committed)
            if right_is_shared {
                inner_shared[idx_right] += arithmetic::mul_public(rs2_field[t], eq2_val);
            } else if right_is_imm {
                inner_public[idx_right] += F::from_i128(imm[t]) * eq2_val;
            }

            // WriteLookupOutputToRD (committed, public): rd * WriteLookupOutputToRD flag
            let write_lookup = (fb & (1u32 << (CircuitFlags::WriteLookupOutputToRD as usize))) != 0;
            if write_lookup {
                inner_public[idx_write_lookup] += F::from_u64(rd_addr[t] as u64) * eq2_val;
            }

            // WritePCtoRD (committed, public): rd * Jump flag
            let is_jump = (fb & (1u32 << (CircuitFlags::Jump as usize))) != 0;
            if is_jump {
                inner_public[idx_write_pc] += F::from_u64(rd_addr[t] as u64) * eq2_val;
            }

            // ShouldBranch (committed, shared): lookup_output if Branch else 0
            let is_branch = (fb & (1u32 << (CircuitFlags::Branch as usize))) != 0;
            if is_branch {
                inner_shared[idx_should_branch] += arithmetic::mul_public(lookup_outputs[t], eq2_val);
            }

            // ShouldJump (committed, public): Jump && !NextIsNoop
            inner_public[idx_should_jump] += F::from_bool(is_jump && !next_is_noop) * eq2_val;

            // Product (mixed)
            match (left_is_shared, right_is_shared) {
                (true, true) => {
                    let k = mul_map[t].expect("mul_map missing entry");
                    inner_shared[idx_product] += arithmetic::mul_public(mul_products[k], eq2_val);
                }
                (true, false) => {
                    let r = if right_is_imm {
                        F::from_i128(imm[t])
                    } else {
                        F::zero()
                    };
                    let prod = arithmetic::mul_public(rs1_field[t], r);
                    inner_shared[idx_product] += arithmetic::mul_public(prod, eq2_val);
                }
                (false, true) => {
                    let l = if left_is_pc {
                        F::from_u64(unexpanded_pc[t])
                    } else {
                        F::zero()
                    };
                    let prod = arithmetic::mul_public(rs2_field[t], l);
                    inner_shared[idx_product] += arithmetic::mul_public(prod, eq2_val);
                }
                (false, false) => {
                    let l = if left_is_pc {
                    ...
                    inner_public[idx_product] += (l * r) * eq2_val;
                }
            }

            // LeftLookupOperand = 0 if Add/Sub/Mul, else LeftInstructionInput
            let is_add = (fb & mask_add) != 0;
            let is_sub = (fb & mask_sub) != 0;
            let is_mul = (fb & mask_mul) != 0;
            if !(is_add || is_sub || is_mul) {
                if left_is_shared {
                    inner_shared[idx_left_lookup] += arithmetic::mul_public(rs1_field[t], eq2_val);
                } else if left_is_pc {
                    inner_public[idx_left_lookup] += F::from_u64(unexpanded_pc[t]) * eq2_val;
                }
            }
            ...
            if is_add {
                ...
                match (left_is_shared, right_is_shared) {
                    (false, false) => {
                        ...
                        inner_public[idx_right_lookup] += (l + r) * eq2_val;
                    }
                    _ => {
                        // Build shared sum = left_share + right_share (+ public pieces)
                        let mut sum = Rep3PrimeFieldShare::zero_share();
                        if left_is_shared {
                            sum += rs1_field[t];
                        } else if left_is_pc {
                            sum = arithmetic::add_public(sum, F::from_u64(unexpanded_pc[t]), party_id);
                        }
                        if right_is_shared {
                            sum += rs2_field[t];
                        } else if right_is_imm {
                            sum = arithmetic::add_public(sum, F::from_i128(imm[t]), party_id);
                        }
                        inner_shared[idx_right_lookup] += arithmetic::mul_public(sum, eq2_val);
                    }
                }
            } else if is_sub {
                // left - right + 2^64
                match (left_is_shared, right_is_shared) {
                    (false, false) => {
                        let l = if left_is_pc { F::from_u64(unexpanded_pc[t]) } else { F::zero() };
                        let r = if right_is_imm { F::from_i128(imm[t]) } else { F::zero() };
                        inner_public[idx_right_lookup] += (l - r + two_pow_64) * eq2_val;
                    }
                    _ => {
                        let mut diff = Rep3PrimeFieldShare::zero_share();
                        if left_is_shared {
                            diff += rs1_field[t];
                        } else if left_is_pc {
                            diff = arithmetic::add_public(diff, F::from_u64(unexpanded_pc[t]), party_id);
                        }
                        if right_is_shared {
                            diff -= rs2_field[t];
                        } else if right_is_imm {
                            diff = arithmetic::sub_shared_by_public(diff, F::from_i128(imm[t]), party_id);
                        }
                        diff = arithmetic::add_public(diff, two_pow_64, party_id);
                        inner_shared[idx_right_lookup] += arithmetic::mul_public(diff, eq2_val);
                    }
                }
            } else if is_mul {
                // RightLookupOperand = Product
                // Reuse Product contribution already accumulated in idx_product by adding again here.
                // (This avoids recomputing per-row products.)
                // We add the same term to RightLookupOperand.
                match (left_is_shared, right_is_shared) {
                    (true, true) => {
                        let k = mul_map[t].expect("mul_map missing entry");
                        inner_shared[idx_right_lookup] += arithmetic::mul_public(mul_products[k], eq2_val);
                    }
                    (true, false) => {
                        let r = if right_is_imm { F::from_i128(imm[t]) } else { F::zero() };
                        let prod = arithmetic::mul_public(rs1_field[t], r);
                        inner_shared[idx_right_lookup] += arithmetic::mul_public(prod, eq2_val);
                    }
                    (false, true) => {
                        let l = if left_is_pc {
                            F::from_u64(unexpanded_pc[t])
            ...
            } else if (fb & mask_advice) != 0 {
                inner_public[idx_right_lookup] += F::from_u64(advice[t]) * eq2_val;
            } else {
                // Otherwise: RightInstructionInput
                if right_is_shared {
                    inner_shared[idx_right_lookup] += arithmetic::mul_public(rs2_field[t], eq2_val);
                } else if right_is_imm {
                    inner_public[idx_right_lookup] += F::from_i128(imm[t]) * eq2_val;
                }
            }
        ...
        for i in 0..n_inputs {
            acc_public[i] += inner_public[i] * eq1_val;
            acc_shared[i] += arithmetic::mul_public(inner_shared[i], eq1_val);
        }
    }
...
mod tests {
    ...
    fn r1cs_claimed_evals_correct() {
        let _tracing_guard = init_tracing("r1cs_inputs_test.json", Path::new("/tmp/co-jolt2-traces"));
        ...
        for (i, (mpc, van)) in opened.iter().zip(vanilla_evals.iter()).enumerate() {
            assert_eq!(mpc, van, "mismatch at input index {i} ({:?})", ALL_R1CS_INPUTS[i]);
        }
    }
}
