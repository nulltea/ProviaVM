...
impl<F: JoltField> Rep3RamReadWriteCheckingWorker<F> {
    pub fn new<PCS: CommitmentScheme<Field = F>>(
        ...
        let prover_state =
            ReadWriteCheckingProverState::initialize(initial_memory_state, K, sm);
                    ...
                    *val_j_0 = checkpoint.to_vec();
                    ...
                    I_chunk
                        ...
                        .for_each(|inc_chunk| {
                            ...
                            evals[0] = evals[0].add(
                                &Rep3Value::Shared(rep3_arith::mul_public(inner[0], eq_r_prime_eval)),
                                party_id,
                            );
                            evals[1] = evals[1].add(
                                &Rep3Value::Shared(rep3_arith::mul_public(inner[1], eq_r_prime_eval)),
                                party_id,
                            );
                        });
                    evals
                    ...
                    I_chunk
                        ...
                        .for_each(|inc_chunk| {
                            ...
                            for j in j_prime << round..(j_prime + 1) << round {
                                ...
                                if let Some(k) = ram_addresses[j] {
                                    let k = k as usize;
                                    if ra[0][k].is_zero() { dirty_indices.push(k); }
                                    ra[0][k] += A[j_bound];
                                }
                            }
                            for j in (j_prime + 1) << round..(j_prime + 2) << round {
                                ...
                                if let Some(k) = ram_addresses[j] {
                                    let k = k as usize;
                                    if ra[0][k].is_zero() && ra[1][k].is_zero() { dirty_indices.push(k); }
                                    ra[1][k] += A[j_bound];
                                }
                            }

                            for &k in dirty_indices.iter() { val_j_r[0][k] = val_j_0[k]; }
                            let mut inc_iter = inc_chunk.iter().peekable();
                            loop {
                                let (row, col, inc_lt, inc) = inc_iter.next().unwrap();
                                debug_assert_eq!(*row, j_prime);
                                val_j_r[0][*col] += *inc_lt;
                                val_j_0[*col] += *inc;
                                if inc_iter.peek().unwrap().0 != j_prime { break; }
                            }
                            for &k in dirty_indices.iter() { val_j_r[1][k] = val_j_0[k]; }
                            for entry in inc_iter {
                                let (row, col, inc_lt, inc) = *entry;
                                debug_assert_eq!(row, j_prime + 1);
                                val_j_r[1][col] += inc_lt;
...
impl<F: JoltField, N: Rep3NetworkWorker> Rep3SumcheckInstanceWorker<F, N> for Rep3RamReadWriteCheckingWorker<F> {
    fn degree(&self) -> usize { DEGREE }

    fn num_rounds(&self) -> usize { self.K.log_2() + self.T.log_2() }
