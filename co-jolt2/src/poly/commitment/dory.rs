        ...
        N: Rep3NetworkWorker,
    {
        ...
        #[cfg(feature = "ring-msm")]
        {
            ...
            for (i, p) in polys.iter().enumerate() {
                if matches!(p.borrow(), Rep3MultilinearPolynomial::Shared(Rep3SharedPoly::CompactRing(_))) {
                    ring_idxs.push(i);
                } else {
                    local_idxs.push(i);
                }
...
fn compute_public_first_reduce_terms(
    ...
    match party_id {
        party_id if owns_first_reduce_d1_left(party_id) => {
            let d1_left =
                multi_pairing_both_affine(&G1Projective::normalize_batch(&v1_pub[..n2]), &g2_affine_all[..n2]);
            (Some(d1_left), None, None, None)
        }
        party_id if owns_first_reduce_d1_right(party_id) => {
            let d1_right = multi_pairing_both_affine(
...
fn rows_to_commitment(
    ...
    let commitment_share = if let Some(g2_cache) = setup.g2_cache.as_ref() {
        ...
        let ml_result = row_commitments_aff
            ...
            .product();
        Bn254::final_exponentiation(MillerLoopOutput(ml_result)).expect("final exponentiation should not fail")
    } else {
...
mod tests {
    ...
    fn dory_u64_scalars_commit_correct() {
        ...
        let polys_by_party: [Rep3MultilinearPolynomial<Fr>; 3] = std::array::from_fn(|pid| {
            ...
            Rep3MultilinearPolynomial::Shared(Rep3SharedPoly::CompactRing(Rep3CompactPolynomial::from_shares(
                shares, shares_bin,
            )))
        });
    ...
    fn dory_u64_scalars_mixed_public_shared_commit_correct() {
        ...
        let polys_by_party: [Rep3MultilinearPolynomial<Fr>; 3] = std::array::from_fn(|pid| {
            let operands: Vec<Rep3Operand> = (0..len)
                ...
                .collect();
            Rep3MultilinearPolynomial::Shared(Rep3SharedPoly::CompactRing(Rep3CompactPolynomial::from_operands(
                operands,
            )))
        });
