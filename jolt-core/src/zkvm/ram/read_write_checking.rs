...
use crate::{
    ...
    poly::{
        ...
        opening_proof::{
            OpeningPoint, ProverOpeningAccumulator, SumcheckId, VerifierOpeningAccumulator, BIG_ENDIAN, LITTLE_ENDIAN,
        },
    },
...
impl<F: JoltField> ReadWriteCheckingProverState<F> {
    fn initialize<PCS: CommitmentScheme<Field = F>, ProofTranscript: Transcript>(
        ...
        let deltas: Vec<Vec<i128>> = trace[..T - chunk_size]
            ...
            .map(|trace_chunk| {
                ...
                for cycle in trace_chunk.iter() {
                    ...
                    let k = remap_address(ram_op.address() as u64, &program_io.memory_layout).unwrap_or(0) as usize;
        ...
        let ram_addresses = trace
            .par_iter()
            .map(|cycle| remap_address(cycle.ram_access().address() as u64, &program_io.memory_layout))
            .collect::<Vec<_>>();
        ...
        checkpoints.push(initial_memory_state.par_iter().map(|x| *x as i128).collect());
...
impl<F: JoltField> RamReadWriteChecking<F> {
    ...
    pub fn new_prover<ProofTranscript: Transcript, PCS: CommitmentScheme<Field = F>>(
        ...
        let prover_state = ReadWriteCheckingProverState::initialize(initial_memory_state, K, state_manager);
                        ...
                        for k in dirty_indices.drain(..) {
                            if !ra[0][k].is_zero() || !ra[1][k].is_zero() {
                                ...
                                inner_sum_evals[0] += ra_evals[0]
                                    .mul_0_optimized(val_evals[0] + self.gamma * (inc_cycle_evals[0] + val_evals[0]));
                                inner_sum_evals[1] +=
                                    ra_evals[1] * (val_evals[1] + self.gamma * (inc_cycle_evals[1] + val_evals[1]));
    ...
    fn phase2_compute_prover_message(&self) -> Vec<F> {
        ...
        let ReadWriteCheckingProverState { inc_cycle, eq_r_prime, ra, val, .. } = self.prover_state.as_ref().unwrap();
            ...
            .map(|j| {
                ...
                let inner_sum_evals = (0..self.K)
                    ...
                    .map(|k| {
                        ...
                        let ra_evals = ra.sumcheck_evals_array::<DEGREE>(index, BindingOrder::HighToLow);
                        let val_evals = val.sumcheck_evals_array::<DEGREE>(index, BindingOrder::HighToLow);

                        [
                            ra_evals[0].mul_0_optimized(val_evals[0] + self.gamma * (inc_evals[0] + val_evals[0])),
                            ra_evals[1].mul_0_optimized(val_evals[1] + self.gamma * (inc_evals[1] + val_evals[1])),
                            ra_evals[2].mul_0_optimized(val_evals[2] + self.gamma * (inc_evals[2] + val_evals[2])),
                        ]
                    })
    ...
    fn phase1_bind(&mut self, r_j: F::Challenge, round: usize) {
        ...
        if round == chunk_size.log_2() - 1 {
            ...
            ra_evals.par_chunks_mut(self.K).enumerate().for_each(|(chunk_index, ra_chunk)| {
                for (j_bound, address) in
                    ram_addresses[chunk_index * *chunk_size..(chunk_index + 1) * *chunk_size].iter().enumerate()
                {
            ...
            *ra = Some(MultilinearPolynomial::from(ra_evals));
            ...
            val_evals.par_chunks_mut(self.K).zip(I.into_par_iter()).enumerate().for_each(
    ...
    fn phase2_bind(&mut self, r_j: F::Challenge) {
        let ReadWriteCheckingProverState { ra, val, inc_cycle, eq_r_prime, .. } = self.prover_state.as_mut().unwrap();
    ...
    fn phase3_bind(&mut self, r_j: F::Challenge) {
        ...
        [ra, val].into_par_iter().for_each(|poly| poly.bind_parallel(r_j, BindingOrder::HighToLow));
    }
}
...
impl<F: JoltField, T: Transcript> SumcheckInstance<F, T> for RamReadWriteChecking<F> {
    ...
    fn expected_output_claim(
        ...
        let (_, ra_claim) = accumulator
            .borrow()
            .get_virtual_polynomial_opening(VirtualPolynomial::RamRa, SumcheckId::RamReadWriteChecking);
        ...
        let (_, inc_claim) = accumulator
            .borrow()
            .get_committed_polynomial_opening(CommittedPolynomial::RamInc, SumcheckId::RamReadWriteChecking);
    ...
    fn output_claim_constraint(&self) -> Option<OutputClaimConstraint> {
        Some(OutputClaimConstraint::sum_of_products(vec![
            ProductTerm::product(vec![
                ...
                ValueSource::opening(OpeningId::Virtual(VirtualPolynomial::RamRa, SumcheckId::RamReadWriteChecking)),
    ...
    fn output_constraint_challenge_values(&self, sumcheck_challenges: &[F::Challenge]) -> Vec<F> {
        ...
        let eq_eval =
            EqPolynomial::mle_endian(self.r_prime.as_ref().unwrap(), &OpeningPoint::<LITTLE_ENDIAN, F>::new(reordered));
