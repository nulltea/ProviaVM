...
impl Term {
    ...
    pub fn pretty_fmt(&self, f: &mut String) -> std::fmt::Result {
        ...
        if coeff == 1 {
            ...
            write!(f, "{}⋅{:?}", coeff, JoltR1CSInputs::from_index(self.input_index))
        }
    }
}
...
impl LC {
    ...
    /// Create an LC from a single input with explicit i128 coefficient (alias).
    ...
    const fn decompose(lc: LC) -> ([Term; 5], usize, i128) {
        let mut terms = [Term { input_index: 0, coeff: 0i128 }; 5];
        let mut len = 0usize;
        let mut c = 0i128;
        match lc {
            LC::Zero => {}
    ...
    pub const fn mul_by_const(self, multiplier: i128) -> LC {
        match self {
            ...
            LC::Terms1Const([t1], c) => {
                LC::Terms1Const([Term::new(t1.input_index, t1.coeff * multiplier)], c * multiplier)
            }
                    ...
                    if let Some(term) = self.term(i) {
                        ...
                        if written_terms > 0 {
                            if term.coeff < 0 {
                                ...
                                let display_term =
                                    Term::new(term.input_index, (-term.coeff));
