...
pub struct DoryProofData {
    pub sigma: usize,
    pub dory_proof_data: ArkDoryProof,
}

#[derive(Clone, Debug, Default, PartialEq, CanonicalSerialize, CanonicalDeserialize)]
pub struct DoryBatchedProof {
    pub proofs: Vec<DoryProofData>,
}
...
impl DoryOpeningProofHint {
    ...
    pub fn into_rows(self) -> Vec<ArkG1> {
        self.0
    }

    pub fn resize(&mut self, new_len: usize, value: JoltG1Wrapper) {
        self.0.resize(new_len, ArkG1(value.0));
    }
}

impl std::ops::Deref for DoryOpeningProofHint {
    type Target = [ArkG1];
...
impl std::ops::DerefMut for DoryOpeningProofHint {
...
impl CommitmentScheme for DoryCommitmentScheme {
    ...
    fn verify<ProofTranscript: Transcript>(
        ...
        dory::verify::<ArkFr, BN254, JoltG1Routines, JoltG2Routines, _>(
            ...
            ark_eval,
            &ark_point,
            &proof.dory_proof_data,
            setup.clone().into_inner(),
            &mut dory_transcript,
        )
        .map_err(|err| ProofVerifyError::DoryError(err.to_string()))?;
...
impl StreamingCommitmentScheme for DoryCommitmentScheme {
    ...
    fn process_chunk<T: SmallScalar>(setup: &Self::ProverSetup, chunk: &[T]) -> Self::ChunkState {
        ...
        let row_commitment = ArkG1(
            G1Projective::msm_field_elements(&g1_bases[..field_chunk.len()], &field_chunk)
    ...
    C::G1: From<ArkG1>,
{
    fn eval_commitment(proof: &Self::Proof) -> Option<C::G1> {
        #[cfg(feature = "zk")]
        {
            proof.dory_proof_data.y_com.as_ref().copied().map(C::G1::from)
        }
        #[cfg(not(feature = "zk"))]
        {
            let _ = proof;
