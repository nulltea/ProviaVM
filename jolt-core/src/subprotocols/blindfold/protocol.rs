...
fn append_label<T: Transcript>(transcript: &mut T, label: &'static [u8]) {
    transcript.append_message(label);
}

fn append_commitments<T: Transcript, G: CanonicalSerialize>(
    transcript: &mut T,
    label: &'static [u8],
    commitments: &[G],
) {
...
fn append_scalars_labeled<T: Transcript, F: JoltField>(
    transcript: &mut T,
    label: &'static [u8],
...
impl<'a, F: JoltField, C: JoltCurve> BlindFoldProver<'a, F, C> {
    ...
    pub fn prove<T: Transcript>(
        ...
        append_label(transcript, b"BlindFold_spartan");
        ...
        for _ in 0..num_vars {
            ...
            append_scalars_labeled(transcript, b"sumcheck_poly", &compressed.coeffs_except_linear_term);
        ...
        append_scalars_labeled(transcript, b"blindfold_az_bz_cz", &[az_r, bz_r, cz_r]);
...
impl<'a, F: JoltField, C: JoltCurve> BlindFoldVerifier<'a, F, C> {
    ...
    pub fn verify<T: Transcript>(
        ...
        append_commitments(transcript, b"blindfold_cross_term", &proof.cross_term_row_commitments);
        ...
        for (round, compressed_poly) in proof.spartan_proof.iter().enumerate() {
            ...
            append_scalars_labeled(transcript, b"sumcheck_poly", &compressed_poly.coeffs_except_linear_term);
