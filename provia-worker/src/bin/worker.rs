...
fn main() -> eyre::Result<()> {
    ...
    let _tracy = if std::env::var("TRACY").is_ok() {
        use provia_worker::utils::memory::start_jemalloc_monitor;
...
fn prove_loop(
    ...
    loop {
        ...
        #[cfg(feature = "test-utils")]
        {
            use provia_worker::utils::tracing::worker_trace_file;

            static TRACING_INIT: OnceLock<()> = OnceLock::new();
            let file = worker_trace_file(my_id, &program_id);
            let _ = TRACING_INIT.get_or_init(|| {
                use provia_worker::utils::tracing::init_tracing_bench;
        ...
        #[cfg(feature = "ring-msm")]
        {
            ...
            let (q0_xlen, q1_xlen, q0_64, q1_64) = provia_worker::poly::commitment::dory::precompute_dapoint_q_columns(
