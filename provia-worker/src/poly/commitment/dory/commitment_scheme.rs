...
#[cfg(all(feature = "ring-msm", not(feature = "rv64")))]
pub(super) type DoryCarryRing = U34;
...
impl<ProofTranscript: Transcript> Rep3CommitmentScheme<Fr, ProofTranscript> for DoryCommitmentScheme {
        ...
        N: Rep3NetworkWorker,
    {
        ...
        let _ = (io_ctx, preproc); // not needed without ring-msm
...
/// Inner implementation of `prove_rep3` shared between feature variants.
#[tracing::instrument(skip_all, name = "Dory::prove")]
pub(super) fn prove_rep3_impl<Network: Rep3NetworkWorker>(
    poly: &Rep3MultilinearPolynomial<Fr>,
    setup: &<DoryCommitmentScheme as CommitmentScheme>::ProverSetup,
    opening_point: &[<Fr as jolt_core::field::JoltField>::Challenge],
    opening_hint: Option<DoryOpeningProofHint>,
    network: &mut Network,
) -> eyre::Result<()> {
...
/// Inner implementation of `combine_hints_rep3` shared between feature variants.
pub(super) fn combine_hints_rep3_impl(
    hints: Vec<MaybeShared<DoryOpeningProofHint>>,
    coeffs: &[Fr],
    _party_id: PartyID,
...
fn commit_shared<ProofTranscript: Transcript, N: Rep3NetworkWorker>(
    ...
    let (num_vars, row_commitments_share) = match shared_poly {
        ...
        Rep3SharedPoly::RingScalars(_) | Rep3SharedPoly::IRingScalars(_) => {
            unreachable!("ring-msm variant requires the ring-msm feature impl")
        }
        Rep3SharedPoly::RLC(_) => {
            unreachable!("RLC polynomials should not be committed directly")
        }
...
#[tracing::instrument(skip_all, level = "trace")]
pub(super) fn rows_to_commitment(
    ...
    let commitment_share = if DoryGlobals::current_context() == DoryContext::Main {
        ...
        <BN254 as PairingCurve>::multi_pair(&row_commitments_wrapped, g2_bases)
    };
    drop(_pairing_span);
...
pub(super) fn rep3_local_coeffs_a(poly: &Rep3DensePolynomial<Fr>) -> (usize, Vec<Fr>) {
