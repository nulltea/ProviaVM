use ark_bn254::Fr;
use provia_worker::poly::commitment::dory::{
    commit_local_rep3, precompute_dapoint_qs, test_support::init_dory_globals, DoryCommitmentScheme, DoryGlobals,
};
use provia_worker::poly::commitment::Rep3CommitmentScheme;
use provia_worker::poly::{Rep3CompactPolynomial, Rep3MultilinearPolynomial, Rep3SharedPoly};
...
fn main() {
    ...
    let (durations_u64, _) = run_rep3_local_test_with_coordinator(
        ...
        move |poly, mut io_ctx: IoContextPool<LocalRep3TestWorkerNet>| {
            ...
            let pool_dir = std::env::temp_dir().join(format!("provia-worker-bench-{}", io_ctx.party_idx()));
