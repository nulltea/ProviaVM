...
use ::guest::{compile_fib, delegate_fib};
use ark_bn254::Fr;
...
fn main() -> eyre::Result<()> {
    ...
    let program = compile_fib(target_dir);

    // Delegate proof to workers
    let n = 9u32;

    info!("delegating proof...");
    let proof_bytes = delegate_fib(&mut client, program, n)?;
    ...
    let inputs = jolt_sdk::postcard::to_stdvec(&n).unwrap();
