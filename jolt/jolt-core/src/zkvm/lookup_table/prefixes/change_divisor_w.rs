...
impl<const XLEN: usize, F: JoltField> SparseDensePrefix<F> for ChangeDivisorWPrefix<XLEN> {
    ...
    {
        let mut result = checkpoints[Prefixes::ChangeDivisorW].unwrap_or(F::from_u64(2) - F::from_u128(1u128 << XLEN));
        if j == XLEN {
    ...
    where
        ...
        F: FieldChallengeOps<C>,
    {
        let updated = checkpoints[Prefixes::ChangeDivisorW].unwrap_or(F::from_u64(2) - F::from_u128(1u128 << XLEN))
            * if j == XLEN + 1 {
                r_x * r_y
            } else if j > XLEN + 1 {
                (F::one() - r_x) * r_y
