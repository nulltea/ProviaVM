...
use crate::jolt::instruction::virtual_movsign::MOVSIGNInstruction;
...
impl TryFrom<&RVTraceRow> for RV32I {
    ...
    fn try_from(row: &RVTraceRow) -> Result<Self, Self::Error> {
        match row.instruction.opcode {
            RV32IM::ADD => Ok(ADDInstruction(row.register_state.rs1_val.unwrap().into(), row.register_state.rs2_val.unwrap().into()).into()),
            RV32IM::SUB => Ok(SUBInstruction(row.register_state.rs1_val.unwrap().into(), row.register_state.rs2_val.unwrap().into()).into()),
            RV32IM::XOR => Ok(XORInstruction(row.register_state.rs1_val.unwrap().into(), row.register_state.rs2_val.unwrap().into()).into()),
            RV32IM::OR  => Ok(ORInstruction(row.register_state.rs1_val.unwrap().into(), row.register_state.rs2_val.unwrap().into()).into()),
            RV32IM::AND => Ok(ANDInstruction(row.register_state.rs1_val.unwrap().into(), row.register_state.rs2_val.unwrap().into()).into()),
            RV32IM::SLL => Ok(SLLInstruction(row.register_state.rs1_val.unwrap().into(), row.register_state.rs2_val.unwrap().into()).into()),
            RV32IM::SRL => Ok(SRLInstruction(row.register_state.rs1_val.unwrap().into(), row.register_state.rs2_val.unwrap().into()).into()),
            RV32IM::SRA => Ok(SRAInstruction(row.register_state.rs1_val.unwrap().into(), row.register_state.rs2_val.unwrap().into()).into()),
            ...
            RV32IM::BLT  => Ok(SLTInstruction(row.register_state.rs1_val.unwrap().into(), row.register_state.rs2_val.unwrap().into()).into()),
            RV32IM::BLTU => Ok(SLTUInstruction(row.register_state.rs1_val.unwrap().into(), row.register_state.rs2_val.unwrap().into()).into()),
            ...
            RV32IM::MULU => Ok(MULUInstruction(row.register_state.rs1_val.unwrap().into(), row.register_state.rs2_val.unwrap().into()).into()),
            RV32IM::MULHU => Ok(MULHUInstruction(row.register_state.rs1_val.unwrap().into(), row.register_state.rs2_val.unwrap().into()).into()),

            RV32IM::VIRTUAL_ADVICE => Ok(ADVICEInstruction(row.advice_value.unwrap().into()).into()),
            RV32IM::VIRTUAL_MOVE => Ok(MOVEInstruction(row.register_state.rs1_val.unwrap().into()).into()),
            RV32IM::VIRTUAL_MOVSIGN => Ok(MOVSIGNInstruction(row.register_state.rs1_val.unwrap().into()).into()),
            RV32IM::VIRTUAL_ASSERT_EQ => Ok(BEQInstruction(row.register_state.rs1_val.unwrap().into(), row.register_state.rs2_val.unwrap().into()).into()),
            RV32IM::VIRTUAL_ASSERT_LTE => Ok(ASSERTLTEInstruction(row.register_state.rs1_val.unwrap().into(), row.register_state.rs2_val.unwrap().into()).into()),
